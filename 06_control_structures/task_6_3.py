...
command_add = ','.join(trunk_values[0])
command_only = ','.join(trunk_values[1])
command_del = ','.join(trunk_values[2])
...
for intf, vlan in trunk.items():
    ...
    for command in trunk_template:
        if command.endswith('allowed vlan'):  
            if vlan[0] is 'add':
                print(' {} add {}'.format(command, command_add[4::]))
            elif vlan[0] is 'del':
                print(' {} remove {}'.format(command, command_del[4::]))
            elif vlan[0] is 'only':
                print(' {} {}'.format(command, command_only[5::]))
        else:     
